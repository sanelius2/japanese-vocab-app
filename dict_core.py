...
def lookup_offline(keyword):
    ...
    def _fetch(form):
        # 优先 is_common=1，其次按释义文本长度降序（释义越丰富越靠前）
        c.execute(
            "SELECT e.id,e.kanji,e.reading,e.meanings,e.is_common "
            "FROM entries e JOIN kanji_idx k ON e.id=k.eid WHERE k.form=? "
            "ORDER BY e.is_common DESC, LENGTH(e.meanings) DESC LIMIT 5",
            (form,))
        rows = c.fetchall()
        if not rows:
            c.execute(
                "SELECT e.id,e.kanji,e.reading,e.meanings,e.is_common "
                "FROM entries e JOIN reading_idx r ON e.id=r.eid WHERE r.form=? "
                "ORDER BY e.is_common DESC, LENGTH(e.meanings) DESC LIMIT 5",
                (form,))
            rows = c.fetchall()
        return rows
